...
class InterceptorsConsumer {
    async intercept(interceptors, dataOrRequest, instance, callback, next) {
        ...
        const result$ = await interceptors.reduce(async (stream$, interceptor) => await interceptor.intercept(dataOrRequest, context, await stream$), Promise.resolve(start$));
